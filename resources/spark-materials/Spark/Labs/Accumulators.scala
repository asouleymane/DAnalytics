// Databricks notebook source exported at Wed, 30 Sep 2015 18:13:47 UTC
// MAGIC %md
// MAGIC #These are the Accumulators and Broadcast Variables :: Scala Lab Exercises
// MAGIC 
...
// MAGIC 
// MAGIC ### The easiest way to do this is with the SparkContext (hereafter just: "sc") textFile(...) method which takes the path of a file on the Databricks server to which you are now logged in... The path that works is: /databricks-datasets/<filename>
// MAGIC 
...
// MAGIC %md
// MAGIC # Great work! 
// MAGIC ### Now, we need an Accumulator instance to work with which we will be sending over to the Function. We get an Accumulator from the SparkContext itself, by calling the Factory [GoF] method accumulator(...) and passing in the initial value for that Accumulator. Try to do that now. In our case, we want to count the number of "blank lines" in the text file:
...
// MAGIC %md #This is the Broadcast Variables Lab Exercise #1
